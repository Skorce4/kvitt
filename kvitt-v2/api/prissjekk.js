...
function analyserPriser(annonser, egenPris, egenKm) {
  ...
  if (egenKm && egenKm > 0) {
    ...
    if (medKm.length >= 1) {
      ...
      ankere = utenKm.slice(0, 3).map((a) => ({ pris: a.pris, faktiskPris: a.pris, km: null, tittel: a.tittel || null, url: a.url || null, selger: a.selger || null, kmDiff: null, justering: 0 }));
    }
  } else {
    // Selgerens km ukjent – vis de tre rimeligste ujustert
    ankere = rene.slice().sort((a, b) => a.pris - b.pris).slice(0, 3)
      .map((a) => ({ pris: a.pris, faktiskPris: a.pris, km: a.km || null, tittel: a.tittel || null, url: a.url || null, selger: a.selger || null, kmDiff: null, justering: 0 }));
  }
